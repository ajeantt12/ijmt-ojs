...
### Step 3b — Security hardening (VPS only, do before OJS install)
...
# Fail2ban — auto-bans IPs with repeated failed SSH logins
apt install -y fail2ban
systemctl enable fail2ban
systemctl start fail2ban

# Disable password SSH login (use SSH key instead — safer)
# First: copy your public key to the server, THEN run this
sed -i 's/#PasswordAuthentication yes/PasswordAuthentication no/' /etc/ssh/sshd_config
systemctl reload sshd

# MySQL: use MEDIUM or STRONG password validation (not LOW as done locally)
# During mysql_secure_installation on VPS, choose 1 (MEDIUM) or 2 (STRONG)
# Also answer Y to: remove test DB, reload privilege tables (both were skipped locally)
```

> **Why this matters:** The VPS is publicly reachable. Without UFW, all ports are open. Without Fail2ban, bots will hammer port 22. Without strong MySQL passwords, a weak password could be brute-forced.
...
# ⚠️  VPS deployment checklist (differences from local run):
#   - Choose MEDIUM (1) or STRONG (2) password validation, not LOW (local used LOW — increase for prod)
#   - Answer Y to "Remove test database" (was accidentally skipped locally)
#   - Answer Y to "Reload privilege tables" (was skipped locally — run manually if needed: sudo mysql -e "FLUSH PRIVILEGES;")
#   - Privilege tables on local install were NOT reloaded; run the flush command above in WSL2 if you hit permission errors
